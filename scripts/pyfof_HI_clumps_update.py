...
def get_fn(model):
	if(model == 'GM3SI1'):
		fn = '/scratch1/06040/adcruz/GMs/GM4SIDM1SFBH/GM4SI1SFBH.1536.'
	elif(model == 'GM3'): 
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243GM4.1536gst1bwK1BH/pioneer50h243GM4.1536gst1bwK1BH.'	
	elif(model == 'GM3noBHs'):
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243GM4.1536gst1bwK1/pioneer50h243GM4.1536gst1bwK1.'
	elif(model == 'GM2'):
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243GM7.1536gst1bwK1BH/pioneer50h243GM7.1536gst1bwK1BH.'
	elif(model == 'GM2SI1'):
		fn = '/scratch1/06040/adcruz/GMs/si1pioneer50h243GM7.1536gst1bwK1BH/si1pioneer50h243GM7.1536gst1bwK1BH.'
	elif(model == 'GM2noBHs'):
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243GM7.1536gst1bwK1/pioneer50h243GM7.1536gst1bwK1.'
	elif(model == 'P0'):
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243.1536gst1bwK1BH/pioneer50h243.1536gst1bwK1BH.'
	elif(model == 'P0noBHs'):
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243.1536gst1bwK1/pioneer50h243.1536gst1bwK1.'
	elif(model == 'GM1'):
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243GM1.1536gst1bwK1BH/pioneer50h243GM1.1536gst1bwK1BH.'
	elif(model == 'GM1noBHs'):
		fn = '/scratch1/06040/adcruz/GMs/pioneer50h243GM1.1536gst1bwK1/pioneer50h243GM1.1536gst1bwK1.'
	elif(model == 'h148'):
		fn = '/scratch1/06040/adcruz/SIMS/h148.cosmo50PLK.3072g3HbwK1BH/h148.cosmo50PLK.3072g3HbwK1BH.003456/h148.cosmo50PLK.3072g3HbwK1BH.' 
	elif(model == 'h148_6144'):
		fn = '/scratch1/06040/adcruz/SIMS/sandraSI/h148.cosmo50PLK.6144g3HbwK1BH/h148.cosmo50PLK.6144g3HbwK1BH.' 
	else:
		print('model isnt currently an option. Add it or try again.')
	return fn

def JC_res_massfunc(model, output, halo, pyfof_pkl): 
	pyfof_grp_min2 = pickle.load(open(pyfof_pkl, "rb" )) 
	fn = get_fn(model)
	...
	h2 = s.halos()
	h21 = h2[halo]
	h21 = h21[h21['amiga.grp'] == halo]
	print('removed substructure')
	pynbody.analysis.halo.center(h21, mode = 'com')
	h21_rcut = h21.g[h21.g['r'].in_units('kpc') > 15]
	print('centered on main halo')
	s2 = cut_gal(s)
	z = s2.properties['z']
	h21_HIcut = h21_rcut[h21_rcut['HIn']  > 4e-08]

	outfn = '../datfiles/' + model + '_clump_masses.dat'
	...
	if os.path.exists(outfn):
		...
		if len(lines) == 1 :
			...
			last_clump_in_file = 0
		else:
			last_clump_in_file = len(lines) - 2
	#creates an outfile if one doesnt already exist
	else:
		last_clump_in_file = 0
		with open(outfn, 'a') as outfile:
			outfile.write('model z clump.grp clump_mass\n')
	print('last_clump_in_file = ', last_clump_in_file)
	
	#g are inds associated with a clump grp 
	i = last_clump_in_file + 1
	for g in pyfof_grp_min2[last_clump_in_file:]: 
		mass = sum(h21_HIcut[g]['mass'])
		with open(outfn, 'a') as outfile: 
			outfile.write('%s %.2f %i %.2f\n' % (model, z, i, mass))
		i = i + 1
...
def clump_grp(model, output, link_len, HI_cut, min_m, halo):
	...
	if(os.path.isfile(outfile)): 
		...
		print('HI cut')
		pyn_data =  np.array(h21_HIcut['pos'])
		print('getting fof grps') 
		pyn_groups = pyfof.friends_of_friends(pyn_data, link_len)
		print('pyn_groups = ', pyn_groups)
		print('cut out grps smaller than minm')
		pyfof_grp_minm = grp_min_m(pyn_groups, min_m)
		print('pyfof_grp_minm = ', pyfof_grp_minm)
		print(model + '_minm_pyfof_grps.pkl')
		file = open(model + "_minm_pyfof_grps.pkl", "wb")
		pickle.dump(pyfof_grp_minm, file)
		print('done pickling')
		pyfof_iord = get_iords(s2.g, h21_HIcut, pyfof_grp_minm)
		print('done getting pyfof iords')
		...
		print('done defining clump.grp and clump.grp.mass')
...
def pyfof_clump_data(model, output, link_len, HI_cut, min_m):
	...
	pyfof_grp_minm = grp_min_m(pyn_groups, min_m)
	pyfof_iord = get_iords(s2.g, h21_HIcut, pyfof_grp_minm)
	print('get fof grp iords for matching')
